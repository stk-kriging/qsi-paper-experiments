...
if POOL > 0
    ...
    parfor it=1:nb_runs
        QSI_SUR(struct, config, it, '../../../data')
    end

    disp('Starting Joint-SUR')
    parfor it=1:nb_runs
        joint_SUR(struct, config, it, '../../../data')
    end

    disp('Starting maximum misclassification')
    parfor it=1:nb_runs
        misclassification(struct, config, it, '../../../data')
    end

    disp('Starting Ranjan')
    parfor it=1:nb_runs
        Ranjan(struct, config, it, '../../../data')
    end

    disp('Starting random sampling')
    parfor it=1:nb_runs
        random(struct, config, it, '../../../data')
    end
